...
- [x] `js/pipeline/isa/IsaInference.js` — derives `{fields, opcodes}` directly from the semantically-named `CU.controlTable` (or the engine's fallback 16-op dispatch when the table is `null`) plus the `IR` node's `opBits/rdBits/rs1Bits/rs2Bits`. Full wire-trace through the datapath turned out to be unnecessary: the CU carries all the read/write/branch/load semantics needed, so inference is a structured read of existing metadata. Graceful fallback to `DEFAULT_ISA` for SUB_CIRCUIT-based CUs (e.g. `cpu-detailed.json`); deeper descent tracked as Tier 14d.
- [x] `js/pipeline/ForwardingDetector.js` — structural pattern match: a MUX whose output feeds `ALU.in_{0|1}`, with one data input traced back to a `REG_FILE_DP` output and at least one other data input to a `PIPE_REG.Q`. Each detected path carries `{id, muxId, aluNodeId, srcNodeId, register:'rs1'|'rs2', srcStage, dstStage, fromStage, toStage}`. The semantic validation of the select-side comparator is deliberately skipped for the MVP; pattern-level detection catches the real cases and stays tolerant to cosmetic wiring variations.
...
- [x] Panel: new **"FORWARDING PATHS"** section below Program Hazards — one row per detected bypass (fromStage badge · register · `PIPE → MUX → ALU` path). Resolved RAWs in Program Hazards now show a `✓ EX→EX` badge in place of the bubble count and the row dims; the header displays `N ✓ resolved`. *Toggle Hide Resolved Hazards* in the Command Palette filters those rows out (session-only). A collapsible `EX→EX ✓ / MEM→EX ✓ / MEM→ID ✗` coverage summary is deferred to Tier 14d once multi-level forwarding detection lands.
- **Example update**: `examples/circuits/pipeline-forwarding-demo.json` — minimal 2-stage datapath (RF → forwarding MUXes → ALU → PIPE_REG(EX/WB) → back to MUX.in1 + RF write-data) with EQ comparators driving the MUX selects. ROM preloaded with `LOAD R1,[R10]` / `ADD R2,R1,R3` / `ADD R4,R2,R3` / `SUB R5,R6,R7` / `HALT`. Inference extracts the ISA; forwarding detection resolves the ADD→ADD RAW; only the LOAD→ADD load-use hazard remains in the report. A full 5-stage pipelined MIPS demo (`mips-forwarding-demo.json`) is deferred to Tier 14d alongside SUB_CIRCUIT CU descent.

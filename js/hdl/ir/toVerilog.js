...
// Emit a single module (top or submodule) into the lines array. Pulled
// out as a helper so Phase-6 hierarchy can emit each submodule above
// the top with the same logic — no duplication of the body-generation
// rules between scopes.
function emitModule(ir, lines) {
...
export function toVerilog(ir, opts = {}) {
  ...
  // collapsing repeats). Verilog requires definitions before use, so
  // top has to land last.
